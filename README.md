...
当前文档覆盖 `YSIFLYADLib 6.0.6`；可运行示例工程见 [YSIFLYADLibSimple](./YSIFLYADLibSimple)（`pod install` 后打开 `YSIFLYADLibSimple.xcworkspace`，演示开屏 / Banner / 插屏 / 自渲染信息流的加载、展示、回调、销毁）。
...
> - 伞头入口：`#import <YSIFLYADLib/YSIFLYADLib.h>`；资源包 `YSAdvSDK.bundle`（已内嵌于 framework）；运行期日志前缀 `[YSAd]`。
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| 6.0.6 | 2026-06-17 | 随上游 IFLYADLib 6.0.6：SDK 内部日志精简 + 日志输出去 IFLY 字眼（前缀 `[YSAd]`，去类名/裸 NSError 域名）；公开 API、能力与交付形态与 6.0.5 一致。 |
| 6.0.5 | 2026-06-17 | 版本线对齐上游主 SDK 6.0.5（同一 SDK 经 rebrand 产出） |
| 1.0.4 | 2026-06-16 | 修复开屏「摇一摇或点击」文案左侧图标显示为白色占位文件图标（改名工具误改资源名 `IFLYAd_shack`→`YSIFLYAd_shack` 与内嵌 bundle 文件失配所致）；附带资源分发治理（统一加载器/跨域兜底/域 bundle 名品牌化）。公开 API、能力与交付形态与 1.0.3 一致。 |
| 1.0.3 | 2026-06-16 | 运行期日志前缀去品牌统一为 `[YSAd]`；公开 API、能力与交付形态与 1.0.2 一致，二进制仅日志字符串变化。 |
...
- **交付形态**：单一 `YSIFLYADLib.xcframework`（**动态 framework**），含 **真机 `arm64` + 模拟器 `arm64`/`x86_64`** 切片，**可直接在模拟器调试**。
- 资源包 `YSAdvSDK.bundle` 与隐私清单 `PrivacyInfo.xcprivacy` 均**内嵌于 framework**，随动态 framework 整体嵌入 app。
- 动态 framework 自动加载全部类与 category，**消费方无需配置 `-ObjC`**，也无需额外资源声明。
...
二进制托管在本仓 **GitHub Releases**（公开可匿名下载），CocoaPods 与 Swift Package Manager 均从 Release 下载同一份动态 `xcframework`。
...
target 'YourApp' do
  ...
  pod 'YSIFLYADLib', :podspec => 'https://raw.githubusercontent.com/LJMcarryu/YSIFLYADLib_iOS/6.0.6/YSIFLYADLib.podspec'
end
```
...
> 说明：
> - `:podspec` 指向 tag `6.0.6` 的 raw podspec，其 `s.source` 是 Release 的 `YSIFLYADLib.xcframework.zip`，CocoaPods 会自动下载解包并链接其中的 `YSIFLYADLib.xcframework`。
> - **请把 URL 钉死到具体 tag（如 `6.0.6`），不要指向分支**；升级版本时同步改 URL 里的 tag。
> - 因是动态 framework，资源与隐私清单随包自动带入，**无需手动加 `-ObjC`**、无需手动拷贝 `.bundle`。

### Swift Package Manager

在 Xcode「**File → Add Packages…**」填入仓库地址，选择版本 `6.0.6`：
...
```swift
dependencies: [
    .package(url: "https://github.com/LJMcarryu/YSIFLYADLib_iOS.git", from: "6.0.6"),
],
targets: [
...
> 说明：
> - SPM 通过 `binaryTarget` 下载 Release 的动态 `xcframework.zip`，由 Xcode 自动嵌入。
> - 与标准版（model A 多模块）不同，**本定制版是单包动态 framework，资源包 `YSAdvSDK.bundle` 与隐私清单已内嵌其中，SPM 接入同样自动带入**，无需手动加资源、无需 `-ObjC`。
...
SDK 已内嵌 Apple 隐私清单（CocoaPods / SPM 接入均自动带入）。**接入方仍须在 App Store Connect 的隐私「营养标签」中如实合并声明以下数据收集，并据 `NSPrivacyTracking = YES` 提供 ATT 授权（见下）。**
...
- `ysifly_setLogEnabled:` 控制日志输出（前缀 `[YSAd]`）：Debug 默认开启、Release 默认关闭；Release 下传 `YES` 可强制开启用于排查。正式发布请关闭。
...
| 现象 | 排查建议 |
| --- | --- |
| `pod install` 找不到 SDK | 确认 `Podfile` 用的是 `:podspec => 'https://raw.githubusercontent.com/LJMcarryu/YSIFLYADLib_iOS/6.0.6/YSIFLYADLib.podspec'`（钉到 tag）。 |
| 广告图片缺失 | 请使用 **1.0.2 及以上**（动态 framework 才会投递内嵌 `YSAdvSDK.bundle`）；早期 1.0.0 静态包已下线。 |
| 开屏「摇一摇或点击」图标显示为白色文件占位 | 1.0.2/1.0.3 的已知缺陷（改名误改资源名致内嵌图标失配），自 1.0.4 起已修复，**请升级到最新版 6.0.6**。 |
