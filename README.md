...
- `ysifly_setLogEnabled:` 控制日志输出（前缀 `[YSAd]`）：Debug 默认开启、Release 默认关闭；Release 下传 `YES` 可强制开启用于排查。正式发布请关闭。自 `6.0.8` 起 SDK 内部日志仅保留**关键节点 `error`**（请求 / 渲染 / 播放 / 监测失败等），`info` / `warn` / 调试 / JSON 日志已整体移除（即便开启也不输出），且不打印内部类名（含 `YSIFLY*`）或裸 `NSError`。
