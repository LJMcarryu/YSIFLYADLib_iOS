...
当前目录对应 `6.3.0` 发布准备，源码和 `Podfile` 均使用 NativeFeed SDK 托管 API；
...
`6.3.0` 正式发布后可在本目录直接消费同版本 tag 与 Release 资产：
...
`Podfile` 已固定到待发布的 `6.3.0` tag，示例工程最低支持 iOS 11.0；Release 公开前
`pod install` 预期不可用：
