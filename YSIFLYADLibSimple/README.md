...
`Podfile` 通过 GitHub Releases 上的 `YSIFLYADLib.podspec` 集成公开发布的 `6.0.5` 版本：

```ruby
pod 'YSIFLYADLib', :podspec => 'https://raw.githubusercontent.com/LJMcarryu/YSIFLYADLib_iOS/6.0.5/YSIFLYADLib.podspec'
```

`6.0.5` 为**动态 framework**：资源包 `YSAdvSDK.bundle` 内嵌其中、随 framework 整体嵌入 app，广告图片可正常加载；**无需在 App target 配置 `-ObjC`**（动态 framework 会自动加载全部类与 category）。
